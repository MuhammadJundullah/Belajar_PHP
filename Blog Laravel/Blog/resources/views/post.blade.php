<x-layout>
  <x-slot:title>{{$title}}</x-slot:title>
    ...
    {{-- <article class="py-8 max-w-screen-md">
      ...
      <div>
        By <a class="text-base text-gray-500 hover:underline" href="/authors/{{$post->author->username}}">{{$post->author->name}}</a> in <a class="text-base text-gray-500 hover:underline" href="/categories/{{$post->category->slug}}">{{$post->category->name}}</a> | <span href="#">{{$post->created_at->diffForHumans()}}</span>
      </div>
  ...
  <div class="flex justify-between px-4 mx-auto max-w-screen-xl ">
      <article class="mx-auto w-full max-w-4xl format format-sm sm:format-base lg:format-lg format-blue dark:format-invert">
          <header class="mb-4 lg:mb-6 not-format">
            <a class="font-medium text-sm text-blue-600 hover:underline" href="/posts">&laquo; Back to all posts</a>
